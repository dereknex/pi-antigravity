  ...
  pi.registerCommand("antigravity.doctor", {
    ...
    handler: async (_args, ctx) => {
      ...
      const lines = [
        ...
        `trailingToolCall=${d.trailingToolCall || "none"}`,
        `lastError=${d.error ? redactSecrets(d.error) : "none"}`,
        "transport=native-streamSimple",
        "runtimeCli=not-used",
        "commands=/antigravity.usage /antigravity.models /antigravity.account /antigravity.image /antigravity.doctor",
      ];
  ...
  pi.registerCommand("antigravity.image", {
    ...
    handler: async (args, ctx) => {
      ...
      if (!parsed.prompt) {
        emitCommandOutput(
          ctx,
          `Usage: /antigravity.image [--ratio ${IMAGE_ASPECT_RATIOS[0]}] [--model ${DEFAULT_IMAGE_MODEL}] [--path file.png] <prompt>`,
          "warning",
        );
        return;
      }
      ...
      try {
        ...
        if (!apiKey) {
          emitCommandOutput(
            ctx,
            `No Antigravity credentials for ${providerId}. Run /login ${providerId} first.`,
            "warning",
          );
          return;
        }
